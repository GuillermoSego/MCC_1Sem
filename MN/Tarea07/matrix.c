...
void InversePowerMethod(double *M, double *B, double *x, int n, double sigma) {
    int maxIterations = 10000;
    double *Ax = (double *)malloc(n * sizeof(double));
    double *tmpM = (double *)malloc(n * n * sizeof(double));
    
    // Ajustamos tmpM a ser (M - sigma * I)
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == j) {
                tmpM[i*n + j] = M[i*n + j] - sigma;
            } else {
                tmpM[i*n + j] = M[i*n + j];
            }
        }
    ...
    // Usamos B como el primer vector estimado
    Divide(B, sqrt(DotProd(B, B, n)), x, n);

    for (int iter = 0; iter < maxIterations; iter++) {
        // Copiamos x a B (guarda el vector anterior)
        for (int i = 0; i < n; i++) {
            B[i] = x[i];
        }

        // Resuelve (M - sigma * I) * Ax = B
        eliminacionGaussiana(n, tmpM, B);
        resuelveSistema(n, tmpM, B, Ax);

        // Actualizamos el vector de estimación x = Ax/norm(Ax)
        Divide(Ax, sqrt(DotProd(Ax, Ax, n)), x, n);

        // Comprueba la convergencia con la función ConditionCheck
        if (ConditionCheck(x, B, n)) {
            break;
        }
    }
