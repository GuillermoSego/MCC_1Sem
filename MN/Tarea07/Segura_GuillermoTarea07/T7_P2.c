...
int main(int argc, char *argv[]){
    ...
    if (rows != cols) {
        printf("La matriz debe ser cuadrada para el método de la potencia inversa.\n");
        free(A);
        return 1;
    }
    ...
    // Pide al usuario un sigma (una aproximación inicial del valor propio que queremos encontrar)
    double sigma;
    printf("Por favor, introduce una aproximación inicial para el valor propio (sigma): ");
    scanf("%lf", &sigma);
    ...
    // Ejecutar el método de la potencia inversa
    InversePowerMethod(A, U, l, rows, sigma);
